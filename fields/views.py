...
#             request, self.template_name, {'meetings': meet}
#         )
...
class AddPlaygroundAdminView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if len(kwargs) >= 3:
            ...
            pid = False
        if request.FILES:
            ans, code = form.uploadPlayground(pid, request.FILES['photo'])
        else:
            ans, code = form.uploadPlayground(pid)
        if ans:
...
class AddPlaygroundView(View):
    ...
    def post(self, request, *args, **kwargs):
        form = self.form_class(request.POST, request.FILES)
        if request.FILES:
            ans, code = form.uploadPlayground(kwargs['ses'], request.FILES['photo'])
        else:
            ans, code = form.uploadPlayground(kwargs['ses'])
        if ans:
...
class ShowPlaygroundView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if len(kwargs) >= 3:
            checkCrit = self.form_class.getCrit(kwargs['sport'], kwargs['price'], kwargs['under'])            
            if checkCrit:
                arr_fields, _ = form.getListPlaygrounds(False, *checkCrit)
                if not(arr_fields):
    ...
    def post(self, request, *args, **kwargs):
        ...
        arr_fields, url_adding = form.getListPlaygrounds(False, sport.id)
        if not(arr_fields):
...
class ShowMapView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        arr_fields, _ = form.getListPlaygrounds(True, sport)
        if not(arr_fields):
...
class ShowPlaygroundMeetingView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        meet_ans, meet_in, meet_date = form_meet.signUpMeeting(kwargs['fid'], kwargs['ses'])
        rate_ans, rate_new = form_rate.ratePlayground(kwargs['fid'], kwargs['ses'])
        if favor_ans:
